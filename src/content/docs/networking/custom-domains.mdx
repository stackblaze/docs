...
<Diagram file="custom-domain-dns.svg" alt="Diagram of CNAME from custom domain to StackBlaze edge with automatic TLS" />
...
If you use Cloudflare, enable "Proxied" mode (orange cloud) for automatic ALIAS-style resolution at the apex. Ensure you're pointing to `my-service.stackblaze.app` as a CNAME with proxy enabled, this is compatible with both subdomains and apex domains.
:::
