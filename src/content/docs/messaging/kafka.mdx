...
<Diagram file="kafka-ha.svg" alt="A producer writes to the partition leader on broker 1. The leader replicates to in-sync replicas on brokers 2 and 3. A consumer reads from the leader. If the leader fails, an in-sync replica is promoted automatically." />
...
<Diagram file="kafka-connect-topology.svg" alt="Source connectors (Postgres CDC and MySQL/MariaDB CDC via Debezium) stream changes into Kafka brokers and topics. Sink connectors (JDBC, S3 on RustFS, MongoDB, ClickHouse) stream records out to those systems." />
