...
3. Readiness probe starts polling the health path (default `/`) every 10 seconds
...
By default, StackBlaze probes `GET /` on the port your service listens on. The probe expects a 2xx HTTP response. A dedicated path such as `/health` is recommended.
