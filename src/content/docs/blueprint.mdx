...
<Diagram file="blueprint-plan-apply.svg" alt="stackblaze.yaml goes through validate (no live reads), plan (diff against live), and apply (upsert only, never deletes). Secrets marked sync false are answered at apply and not stored in the file." />
