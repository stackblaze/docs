---
title: ASP.NET Core
description: Deploy ASP.NET Core on StackBlaze from GitHub or a Dockerfile.
---

Add a service from your ASP.NET Core repo. Nixpacks detects **csharp** from `*.csproj`. A `Dockerfile` in the service root always wins.
...
Override install / build / start on the service if detection is wrong. Typical start: `dotnet MyApp.dll`.
