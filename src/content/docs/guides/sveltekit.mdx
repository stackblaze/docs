---
title: SvelteKit
description: Deploy SvelteKit on StackBlaze from GitHub or a Dockerfile.
---

Add a service from your SvelteKit repo. Nixpacks detects **node** from `package.json`. A `Dockerfile` in the service root always wins.
...
Override install / build / start on the service if detection is wrong. Typical start: `node build`.
