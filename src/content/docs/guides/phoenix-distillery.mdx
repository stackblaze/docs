---
title: Phoenix + Distillery
description: Deploy Phoenix + Distillery on StackBlaze from GitHub or a Dockerfile.
---

Add a service from your Phoenix + Distillery repo. Nixpacks detects **elixir** from `mix.exs`. A `Dockerfile` in the service root always wins.
...
Override install / build / start on the service if detection is wrong. Typical start: `the release binary`.
