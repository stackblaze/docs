---
title: Play
description: Deploy Play on StackBlaze from GitHub or a Dockerfile.
---

Add a service from your Play repo. Nixpacks detects **scala** from `build.sbt`. A `Dockerfile` in the service root always wins.
...
Override install / build / start on the service if detection is wrong. Typical start: `the staged binary`.
