---
title: Angular
description: Deploy Angular on StackBlaze from GitHub or a Dockerfile.
---

Add a service from your Angular repo. Nixpacks detects **node** from `package.json`. A `Dockerfile` in the service root always wins.
...
Override install / build / start on the service if detection is wrong. Typical start: `node dist/*/server/server.mjs`.
