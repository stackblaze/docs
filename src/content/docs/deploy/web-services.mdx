---
title: Web Services
description: Deploy HTTP services with automatic TLS, health checks, and zero-downtime rolling updates.
---
...
A web service is any process that listens on a TCP port and serves HTTP(S) traffic. StackBlaze provisions a public URL in the form `https://your-service.stackblaze.app`. You can also attach a [custom domain](/networking/custom-domains/).

Web services support HTTP/1.1, HTTP/2, and WebSockets. There is no need to terminate TLS yourself — the platform handles it and forwards plain HTTP to your container.
