...
description: Always-on services with no public domain — queue consumers, processors, and other private workloads.
---
...
<Diagram file="worker-queue.svg" alt="Public HTTP enqueues a job to Redis. Private worker replicas with no ingress consume from the same queue and scale independently." />

## When to leave a service private
...
For deeper observability, emit structured JSON logs and open [Coroot](/operations/observability/) from the service.

## Related

If the work should run on a schedule, use a [cron service](/deploy/cron-jobs/). For disk that survives restarts, attach a [volume](/storage/persistent-disks/).
