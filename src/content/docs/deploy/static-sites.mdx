---
title: Static sites
description: Build a frontend and serve the output as a StackBlaze service.
---

A static site is still a **service**. StackBlaze runs the build, then serves the output over HTTP with TLS. If you need server-side rendering, API routes, or a database at request time, use [public HTTP](/deploy/web-services/) on a runtime service instead.
