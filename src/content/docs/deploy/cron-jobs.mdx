...
At the scheduled time, StackBlaze creates an isolated run that executes your container, runs the start command, and then terminates. Each run is completely isolated — no shared state between runs except what you persist to a database or object storage.
