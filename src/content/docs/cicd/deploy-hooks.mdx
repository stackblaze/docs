...
```bash title="terminal"
# Queue a build for project/phase/app
...
- **Build queue**: `POST /api/deployments/build/{pipeline}/{phase}/{app}` starts or queues a build for that app.
- **Phases**: production and review (or a custom phase) are separate namespaces. Address them in the path, not with a hook query string.
:::
